...
| --- | --- | --- |
| `capwin.py TITLE OUT [x0 y0 x1 y1 [pad]]` | `PrintWindow` capture of a top-level window found by exact title (`Lab02` is the Pro main window; `Classify` a floating dialog). Works even when another window covers it. | **Default.** Anything inside the Pro main window: ModelBuilder canvas, docked panes, the Geoprocessing pane, the map. |
