...
link to the course site, plus the two plain-text lab mentions. The weekly schedule pages of work
item 3 were built in the repo in September 2026; what remains is optional: adding the "Lecture
materials:" line to Learning Suite (see work item 3), and a decision on the example-project files.
...
| Lecture page (optional, see below) | `…/ce414-gis-applications/schedule/week-NN/` | built September 2026 |
...
Week numbers agreed with `docs/schedule/README.md`, so no renumbering was needed. The slugs below
...
## Work item 3 — lecture pages (only the Learning Suite edit remains)
...
**Option B is now built (September 2026):** the site was restructured around one page per week —
`docs/schedule/week-NN.md`, generated by `tools/build_schedule.py` — each linking that week's
Tuesday and Thursday lecture decks, the lab due, and study guides once they exist. The nav's old
`Lectures` menu was folded into `Schedule`. What is still open is the Learning Suite side: adding a
second "Lecture materials:" line (→ `…/ce414-gis-applications/schedule/week-NN/`) to each schedule
day, alongside the existing deck link. That is a Learning Suite edit, not a repo change, and does
not block anything.
...
1. **Add the "Lecture materials:" line to each schedule day** (work item 3) — the weekly pages it
   would link to now exist under `docs/schedule/`.
