# Lab 9: Practicing with Interpolation
...
![Diagram of fixed-radius local averaging: an input sample layer of scattered points above an interpolated raster surface, with one output cell value averaged from three samples inside the search radius and another based on a single sample.](images/lab09-fixed-radius-interpolation-concept.png)
