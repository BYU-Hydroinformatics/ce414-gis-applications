...
>
> **Figures.** Every figure on this page except the example map at the bottom was captured in
> ArcGIS Pro 3.7.1 on September 6, 2026, from the run described above: fourteen new captures plus
> re-shoots of all eight old ModelBuilder and tool-dialog figures. The example map is still the
> old one and is flagged where it appears.
...
This is an example of a Utah map result. Make sure to create two maps: one for Utah County and one for your second scene.
...
<!-- STALE: this is the old example layout from the Word handout, not re-made from the 2025 scene, and its text box still reads "NDVI Lab / Date / Projection". Re-make it with tools/lab01/build_layouts.py as a pattern (arcpy.mp against C:\Ames\Lab02\Lab02.aprx) before this draft is assigned. -->
