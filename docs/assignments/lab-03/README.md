...
for you. These are the tools you will use for the first time in this course. The icon beside each
one is a reminder of what it does to your data: the orange part is what comes out.

| Tool | What it does |
| --- | --- |
| ![Georeference icon: a tilted scanned sheet swinging into a north-up dashed frame](images/icon-georeference.svg){ .tool-icon }<br>**Georeference** (Imagery tab, Alignment group) | Opens the Georeference tab for the selected raster layer. Everything below lives on that tab |
...
| ![Add Field icon: an attribute table gaining a new empty column marked with a plus](images/icon-add-field.svg){ .tool-icon }<br>**Add** (attribute table, Field group) | Adds a field, so your features can carry a name |
...
FIGURES A TO E (2026-09-21, instructor request for a more visual page). A and D are generated
illustrations from the OpenAI image skill (gpt-image-1, no text in either; the only glyph is the
"N" on D's north arrow) saved as lab03-paper-to-gis.jpg and lab03-plan-vs-birds-eye.jpg. B is a
Pillow composite of two icons generated for the Week 4 deck (slides/week-04/images/geo-word-*.jpg).
C is a composite of the Week 4 deck's control-point panels, which are Pillow overlays on one
generated line-art map (the drift on the left panel is a 3-degree rotation about the cluster
centroid, illustrative, not a solved transformation). E is the deck's hydrograph figure from
tools/week04_hydrograph_svg.py with the animation style block stripped and a white background
added so it reads in the dark theme; its numbers are made up to make the point and are not
measurements. The tool-table icons are the ones tools/lab03/make_svgs.py already generated for
the draft. The same five figures are in lab3-draft.md at the same anchors.
