# Lab 11: Least Cost Path Power Line Analysis
...
![Full ModelBuilder diagram for the least cost path analysis, running from the input shapefiles on the left through select, clip, buffer, raster conversion and reclassify steps to the Raster Calculator, Cost Distance, Cost Path and Raster to Polyline tools on the right](images/lab11-full-model-overview.png)
...
![Mosaic To New Raster tool in the ModelBuilder canvas, with four 30 m DEM tiles and the project geodatabase as inputs and CombinedRaster as the output](images/lab11-mosaic-to-new-raster-model.png)
...
![ModelBuilder canvas showing the Counties layer flowing through Select Utah County and Buffer to a Utah County Buffer output, with Streams NHD and Lakes NHD each flowing through a Select tool](images/lab11-select-buffer-county-model.png)
...
![ModelBuilder canvas showing Intersect tools applied to streams, lakes, municipalities and electrical lines, and a Clip tool applied to the UDOT routes, each against the Utah County Buffer](images/lab11-clip-intersect-model.png)
...
![ModelBuilder canvas with Buffer tools on the clipped streams, lakes and UDOT routes, and a Multiple Ring Buffer tool on the clipped municipalities](images/lab11-buffer-tools-model.png)
...
![Multiple Ring Buffer tool Parameters tab with UCO Municipalities as input, distances 1 through 5, Buffer Unit set to Kilometers, field name distance, and Dissolve Option set to Non-overlapping rings](images/lab11-multiple-ring-buffer-parameters.png)

**Figure 6.** Parameters tab of the Multiple Ring Buffer tool.

![Multiple Ring Buffer tool Environments tab with Extent set to As Specified Below and numeric extent values filled in](images/lab11-multiple-ring-buffer-environments.png)
...
![Polygon to Raster tool Parameters tab with Buffered Lakes as the input feature, OBJECTID as the value field, cell assignment type of Cell center, and Cellsize set from CombinedRaster](images/lab11-polygon-to-raster-parameters.png)

**Figure 8.** Parameters tab of the Polygon to Raster tool.

![Polygon to Raster tool Environments tab with Extent set to As Specified Below and Cell Size set to 100](images/lab11-polygon-to-raster-environments.png)
...
![ModelBuilder canvas showing five Polygon to Raster and Polyline to Raster tools producing the streams, lakes, roads, municipalities and power line rasters, each feeding a Reclassify tool](images/lab11-raster-conversion-reclassify-model.png)
...
![Reclassify tool for the streams raster, mapping value 1 to 10 and NODATA to 1](images/lab11-reclassify-streams.png)

**Figure 11.** Reclassify tool in ModelBuilder for the streams raster.

![Reclassify tool for the lakes raster, mapping value 1 to 10 and NODATA to 1](images/lab11-reclassify-lakes.png)
...
![Reclassify tool for the roads raster, mapping the value range 1 to 236 to 1 and leaving NODATA as NODATA](images/lab11-reclassify-roads.png)
...
![Reclassify tool for the municipalities raster, mapping ring values 1 through 5 to 10, 8, 6, 4 and 2 respectively and NODATA to 1](images/lab11-reclassify-municipalities.png)
...
![Reclassify tool for the power lines raster, mapping value 1 to 1 and NODATA to 10](images/lab11-reclassify-power-lines.png)
...
![ModelBuilder canvas showing the five reclassified rasters and CombinedRaster feeding the Raster Calculator tool, which outputs the Powerline Use Rating raster](images/lab11-raster-calculator-model.png)
...
![ModelBuilder canvas showing the Powerline Use Rating raster and Windfarm.shp feeding the Cost Distance tool, which outputs a cost distance raster and a cost backlink raster](images/lab11-cost-distance-model.png)

**Figure 17.** The Cost Distance tool in ModelBuilder.

![Cost Distance tool window with Windfarm.shp as the source, Powerline Use Rating as the input cost raster, and outputs for the distance raster and the backlink raster](images/lab11-cost-distance-tool.png)
...
![ModelBuilder canvas showing the cost distance raster, cost backlink raster and DataCenter.shp feeding the Cost Path tool, which outputs the Cost Path Raster](images/lab11-cost-path-model.png)

**Figure 19.** The Cost Path tool in ModelBuilder.

![Cost Path tool window with DataCenter.shp as the destination, Id as the destination field, the cost distance and cost backlink rasters as inputs, and Path type set to Each cell](images/lab11-cost-path-tool.png)
...
![ModelBuilder canvas showing the Cost Path Raster feeding the Raster to Polyline tool, which outputs the Least Cost Path feature class](images/lab11-raster-to-polyline-model.png)
...
![Example finished layout titled "Least Cost Path Power Lines", showing the computed path in red running from the wind farm south-east of Utah Lake north to the data center, with a north arrow, scale bar, legend, and a circular inset map of the Powerline Use Rating cost surface](images/lab11-example-map.png)
...
images renamed from fig-NN:
  fig-01 -> lab11-full-model-overview.png
  fig-02 -> lab11-mosaic-to-new-raster-model.png
  fig-03 -> lab11-select-buffer-county-model.png
  fig-04 -> lab11-clip-intersect-model.png
  fig-05 -> lab11-buffer-tools-model.png
  fig-06 -> lab11-multiple-ring-buffer-parameters.png
  fig-07 -> lab11-multiple-ring-buffer-environments.png
  fig-08 -> lab11-polygon-to-raster-parameters.png
  fig-09 -> lab11-polygon-to-raster-environments.png
  fig-10 -> lab11-raster-conversion-reclassify-model.png
  fig-11 -> lab11-reclassify-streams.png
  fig-12 -> lab11-reclassify-lakes.png
  fig-13 -> lab11-reclassify-roads.png
  fig-14 -> lab11-reclassify-municipalities.png
  fig-15 -> lab11-reclassify-power-lines.png
  fig-16 -> lab11-raster-calculator-model.png
  fig-17 -> lab11-cost-distance-model.png
  fig-18 -> lab11-cost-distance-tool.png
  fig-19 -> lab11-cost-path-model.png
  fig-20 -> lab11-cost-path-tool.png
  fig-21 -> lab11-raster-to-polyline-model.png
  fig-22 -> lab11-example-map.png
...
stale/unverified screenshots:
  - lab11-full-model-overview.png (Figure 1) — illegible at page width; needs re-export
